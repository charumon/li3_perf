...
Dispatcher::applyFilter('run', function($self, $params, $chain) {
    ...
    if (
      ...
      $css = $View->render('template', array(), array(
          'template' => 'css',
          'library'  => 'li3_perf',
        )) . '<!-- PLUGIN STYLES -->';

      $js = $View->render('template', array(), array(
          'template' => 'js',
          'library'  => 'li3_perf',
        )) . '<!-- PLUGIN SCRIPTS -->';

      // Inserts css and js.
      $result->body[0] = str_replace(array(
        '<!-- PLUGIN STYLES -->', '<!-- PLUGIN TOOLBAR -->', '<!-- PLUGIN SCRIPTS -->'
        ), array($css, $toolbar, $js), $result->body[0]
      );
    }
